...
from aal.domains import domain_skill_paths, load_domains, resolve_read_target
from aal.parser import parse_aal_file
from aal.scanner import iter_source_files, read_file_for_parse
...
def cmd_verify_domains(
    ...
    *,
    ...
    if not domains:
        print("ERROR: no domains defined — run: aal install --project --hooks")
        return 1
    ...
    if strict and pkg_version:
        skills_pkg = config.get("skills_package", "consumer")
        installed = "unknown"
        if skills_pkg and skills_pkg not in ("consumer", "unknown"):
            try:
                import importlib

                mod = importlib.import_module(skills_pkg.replace("-", "_"))
                installed = getattr(mod, "__version__", "unknown")
            except Exception:
                installed = "unknown"
        else:
            from aal import __version__ as aal_version

            installed = aal_version
        if pkg_version != installed:
            print(
                f"WARN: .cursor/aal.yaml records skills_package_version={pkg_version!r} "
                f"but installed {skills_pkg!r} is {installed!r}"
            )
            violations += 1
