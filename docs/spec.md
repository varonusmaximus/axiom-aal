...
| **IDE — validate** | **Yes** (same hook or pre-commit) | `aal verify-domains` | Ensures annotations resolve; blocks if not |
...
| **CI/CD** | Encouraged (week 2+) | Pinned package + `aal doctor --strict` + `aal verify-domains --strict` | Validates **committed** registry, skills, and annotations |
...
**On inject:** `aal resolve --file PATH --line N` parses the whole file (or uses a cached parse) to resolve effective domain(s) at line *N*.
...
| `summary` | No | One-line description for `aal explain` |
...
**Path:** `.cursor/bootstrap.yaml` (repo-authored; not replaced by `aal upgrade`)

Used by `aal bootstrap suggest` and `aal bootstrap apply --level file` to map legacy files → domains.
...
AAL provisioning produces **repo artifacts that must be committed to git**. CI validates the checked-out tree — it does not run a fresh `aal install` on every PR.
...
| **Commit after `aal install` / `aal upgrade`** | PR reviewers see skill and config changes with dependency bumps |
...
The user does **not** run CLI commands during edit. Failed inject (missing skill file) surfaces as hook error; fix with `aal verify-domains` or re-run `aal upgrade` if files are missing from the committed tree.
...
Returns skill paths and content for injection. **`aal resolve` must fail** if validation would fail (unknown domain, missing skill) — same rules as `verify-domains`. See [AAL-deployment-guide.md](./AAL-deployment-guide.md) §3 and §6.
...
Shipped via `pip install axiom-aal`. Implementation follows [AAL-implementation-backlog.md](./AAL-implementation-backlog.md).

| Command | Purpose |
|---------|---------|
| `aal install [--hooks]` | Bootstrap manifest paths into repo (project-scoped); write `.axiompy-manifest.json` |
| `aal upgrade [--force]` | After pip upgrade: replace all manifest paths from package; preserve `*.override.md` |
| `aal uninstall` | Remove manifest-tracked paths; preserve overrides and `domains.local.yaml` |
| `aal init FILE --domain NAME` | Create **new** file with `# @!NAME` |
| `aal annotate FILE --domain NAME` | Add file-level annotation to **existing** file if missing (idempotent) |
| `aal bootstrap suggest` | Scan `bootstrap.yaml` P0 globs → suggested domain(s) + warnings |
| `aal bootstrap apply --level file` | Bulk insert file-level annotations only; `--dry-run` default |
| `aal annotate FILE --function NAME --domain D` | **Post-MVP:** Function-level override (phase 2) |
| `aal bootstrap refine FILE` | **Post-MVP:** Prompt for function-level customization |
| `aal bootstrap report --mixed` | **Post-MVP:** Files needing phase-2 review |
| `aal coverage report` | **Post-MVP:** Unannotated P0 paths |
| `aal resolve --file PATH [--line N]` | JSON: domains, skills, inject content (for hooks) |
| `aal explain FILE [--line N]` | Human-readable resolution |
| `aal verify-domains [--strict] [--files PATH...]` | **Shared validator** — annotations, placement, overrides, skills on disk |
| `aal refresh --check` | Optional preview diff vs package before upgrade |
| `aal doctor [--strict]` | Installation health; version drift vs `aal.yaml` |
| `aal override init --domain D --target T` | Scaffold `{target}.override.md` |
| `aal impact DOMAIN` | Files referencing a domain |
| `aal skills` | List bundled domains |
...
|---------|------|---------|-----------|
| **Inject hook** | Before inject on Write/Edit | `aal resolve` (internally validates) or explicit `verify-domains` | Yes |
| **Pre-commit** | Before commit | `aal verify-domains --strict` | Recommended |
| **CI/CD** | On PR / push | `aal doctor --strict` then `aal verify-domains --strict --files <changed>` | Encouraged |
...
5. With `--strict`: `skills_package_version` matches installed package (`aal doctor --strict`)
6. Paths under `ignore_dirs` are skipped (§4.6)

Optional (§3): if guards present, validate `then=read:domain` targets resolve.

**CI contract:** checkout repo → `pip install -r requirements-dev.txt` → `aal doctor --strict` → `aal verify-domains --strict --files $(git diff --name-only ...)`. CI validates **changed files** in the PR (not necessarily the entire monorepo). Optional full-repo verify on main/nightly. CI does **not** run `aal install` or inject.
...
`pip install -U` updates tooling in `site-packages` only. **`aal upgrade`** replaces every path listed in `.cursor/.axiompy-manifest.json` from the current package bundle.

| Path class | On `aal upgrade` |
|------------|------------------|
...
Repos run `aal install --hooks`, **commit** `.cursor/` and `.cursor/`, and pin `axiompy` in `requirements-dev.txt`. Package upgrades use `aal upgrade`; company rules use `<domain>.override/SKILL.md` (§5.1).
