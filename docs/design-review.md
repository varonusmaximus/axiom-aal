...
**Answer:** **`preToolUse` inject hooks** (committed `.cursor/hooks/`). Rules alone are probabilistic. Default: `enforcement: inject` in `.cursor/aal.yaml`. Same validation via `aal verify-domains`.
...
**Answer:** Edit skills in `.cursor/skills/` freely (or use `<domain>.override/SKILL.md`). No `freeze`. After package bump: `aal upgrade` + commit.
...
**Answer:** **`pip` updates tooling only.** Run **`aal upgrade`** to replace all manifest paths from package. **`*.override.md`** and `domains.local.yaml` preserved. Commit `.cursor/` + `.cursor/` after upgrade. No selective merge — full overwrite of bundled assets.
...
**Answer:** Distribution name vs short CLI (normal Python packaging). Mitigate with `aal doctor`, `python -m aal`, optional `axiompy` alias. Hooks use venv-safe invocation. **Implementation target:** axiompy / `aal` (placeholders in spec docs).
...
|------------|-----------|
| Skills → edits | **Hooks** (`preToolUse` → `aal resolve` → inject) |
...
**Original grill wording:** "Rules, hooks, or CI — pick one." **Reframed (v1.3):** `aal install --hooks` ships **hooks + host glue + skills + registry** as one committed bootstrap. Real fork:

| Option | What you ship |
|--------|----------------|
| **A — Local install** | `aal install --hooks` → commit `.cursor/` + `.cursor/` → annotate P0 → verify inject |
...
| **Bootstrap phase 1** | `bootstrap suggest`, `bootstrap apply --level file`, `annotate` (file-level), `aal init` (new files) |
