...
> **Status:** MVP shipped — parser, domains, verify, resolve, install, bootstrap, doctor, **Cursor inject hook** (`cursor_hook.py`), CI gate, and dogfood annotations on this repo.
...
## Phase 3 — Inject-on-edit (MVP) — implemented

| Spec section | Module | Deliverable |
|--------------|--------|-------------|
| §7 Inject | `middleware.py` | `aal_inject_on_edit()`, `EditIntent` (SDK/headless API) |
| §7 Cursor host | `cursor_hook.py` | `run_cursor_pretooluse()` — stdin JSON, stdout permission JSON |
| §6.1 Modes | — | **Post-MVP:** `inject` / `cache` / `deny` from `aal.yaml`; `session.py` |
| Session cache | — | **Post-MVP:** `.cursor/.aal-session.json`; mtime invalidation |

**Bundled hook:** `.cursor/hooks/aal-inject.sh` → `aal hook cursor-pretooluse` (venv-safe). Parses Cursor preToolUse stdin; fail-closed on resolve errors.
...
- CI running `aal install` instead of validating committed tree
