...
`aal install --hooks` writes **project-scoped, committable** artifacts:
...
| `.cursor/.axiompy-manifest.json` | Paths replaced by `aal upgrade` | **Yes** |
...
`aal install --hooks` provisions tooling; it does **not** annotate legacy files.

**Phase 1 — File-level (automatic):** Scan P0 paths → suggest domain from path heuristics → `aal bootstrap apply --level file` inserts one `# @!domain` at each file top. Homogeneous files are done; inject works on the next edit.

**Phase 2 — Function-level (prompted):** `aal bootstrap refine` flags mixed-concern files; tooling prompts which functions need overrides (§4.2). User or agent confirms — do not bulk-auto-annotate functions.

| Scenario | Phase |
|----------|-------|
| Whole repo P0 tree | Phase 1 bulk, then Phase 2 on flagged files |
| New file | `aal init` — file-level at create |
...
4. CI runs `aal doctor --strict` and `aal verify-domains --strict` on every PR.
5. Package upgrades use `aal upgrade` and commit refreshed manifest paths.
...
| PR includes manifest refresh with dependency bump | Run `aal install` in CI instead of validating commits |
...
| Phase | Goal | Actions |
|-------|------|---------|
| **1 — Local bootstrap (week one)** | Full committed install | `aal install --hooks` → commit `.cursor/` + `.cursor/` (hooks, rules glue, skills, registry) → annotate P0 paths → smoke-test inject |
| **2 — Coverage** | Expand annotations | `aal init --domain code-review` on auth/crypto/migrations |
...
**`.cursor/rules/aal.mdc`** — **host-specific glue**, installed with `aal install --hooks` alongside the inject hook. Not policy — hooks connect skills to edits; rules tell agents how to participate in AAL on Cursor (respect annotations, trust inject). Do not put domain policy here — that belongs in `.cursor/skills/`.
...
Install with: `aal install --hooks`. **Commit** `hooks.json` and the script.
...
End pipeline with `aal verify-domains --strict`.
...
- After package upgrade: `aal upgrade --force && git commit`.
```
...
CI validates the **committed repository tree** with a **pinned** package. It does **not** run `aal install` on every PR. **`verify-domains` scans changed files in the PR**, not the entire monorepo on every run.
...
1. `aal install --hooks` — commit hooks + skills
...
Add `<domain>.override/SKILL.md` beside bundled skill (spec §5.1). Commit the override file. **`aal upgrade` never removes overrides.**
...
|--------------|--------------|
| Skip `aal upgrade` after pip bump | `doctor --strict` / CI fails on version metadata; inject may use stale bundled skills |
...
5. After package upgrade: `aal upgrade` + commit.
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| Agent ignores skills | Hooks not committed or Cursor not loading `.cursor/hooks.json` | `aal install --hooks`; commit `.cursor/` |
| Hook errors on write | Missing skill file in git | `aal verify-domains`; ensure `.cursor/skills/` committed |
...
| CI fails doctor | Package bump without `aal upgrade` | Run upgrade; commit manifest + requirements |
| Stale skills after upgrade | Upgrade not committed | `aal upgrade --force`; commit `.cursor/` |
| Wrong skills on edit | Precedence | `aal explain FILE --line N` |
